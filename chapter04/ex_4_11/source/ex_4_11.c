...
int getop(char s[])
{
    ...
    do {
        if (last_char == '\0')
        {
            // If last_char was not set, read c from the input
            c = getch();
        }
        else
        {
            // Otherwise, c is equal to the last char,
            // Then we can reset the last char
            c = last_char;
            last_char = '\0';
        }
    } while ((s[0] = c) == ' ' || c == '\t');
    ...
    // Carry on with the rest of the algorithm
    s[1] = '\0';
